...
static int _modbus_set_slave(modbus_t *ctx, int slave)
{
    ...
    if (slave >= 0 && slave <= 247) {
        ...
        /* The special value MODBUS_RTUTCP_SLAVE (0xFF) can be used in RTUTCP mode to
...
static int _modbus_rtutcp_filter_request(modbus_t *ctx, int slave)
{
    return _modbus_rtu_filter_request(ctx, slave);
}
